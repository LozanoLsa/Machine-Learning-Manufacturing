...
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/LozanoLsa/Visual_Defects_Are_Not_Random/blob/main/02_Naive_Bayes_Surface_Defect_Inspection.ipynb)
...
With a **21.6% defect rate** across 1,847 painted parts, the question isn't whether defects are costly — it's whether the process signals that precede them are detectable. They are.

---

## 📊 Dataset

- **1,847 painted plastic part records** from injection-to-inspection production line
- **Target:** `surface_defect` (binary) — 1 = visual defect detected at final inspection
- **Class balance:** 21.6% defective (moderate imbalance)
...
| `num_handlings` | 0.169 | Strong numeric driver |
| `part_protection` | Dominant (MI) | Unprotected: 29.3% vs Protected: 15.9% |
| `container_type` | Secondary (MI) | Cardboard: 29.5% vs Plastic box: 15.1% |
...
> 📦 **Full Project Pack** — complete dataset (1,847 records), notebook with full outputs, presentation deck (PPTX + PDF), and `app.py` part inspection simulator available on [Gumroad](https://lozanolsa.gumroad.com).
